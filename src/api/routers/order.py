...
@router.post(
    "/items/{order_item_id}/customizations",
    response_model=OrderItemCustomizationResponse,
    status_code=201,
)
def add_customization_to_order_item(
    ...
    result = service.add_customization_to_order_item(
        order_item_id=order_item_id, customization_id=customization.customization_id
    )
    if not result:
        raise HTTPException(
            status_code=404, detail="Order item or customization not found"
        )
    return result
